...
The software to-dos this week take two forms: the setup function
```setup_joystick()``` and reusing the ```timer_setup()``` and ```setup_lcd()```
and changes to the main loop, where we will poll the ADC for data, move the ship, and scroll the game screen!

All of the game logic and screen updates are given to you, the only things you need to fill in are the setup function in ```helpers.c```, the calls to setup functions in ```app_main```, and the adc reading logic in ```app_main```.
